...
pub fn save_company<T: Config>(
    ...
    // 将公司数据存入到公司表中
    // println!("1. save_company(): 公司表长度: {:?}", company_vec.len());
    company_vec
        .try_push(company.clone())
        .expect("Failed to push company data");
    // println!("2. save_company(): 公司表长度: {:?}", company_vec.len());
    company_id
}
...
fn gen_company_id<T: Config>(
    ...
    // println!("1. gen_company_id(): 公司表长度：{:?}", company_vec.len());
    let len_encoded: Vec<u8> = (company_vec.len() as u64 + 1).to_le_bytes().to_vec();
    // println!("2. gen_company_id(): 公司表长度：{:?}", company_vec.len());
    // println!("3. gen_company_id(): len_encoded: {:?}", len_encoded);
